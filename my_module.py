...
english_speaking_countries = [5309, 5502, 5303, 5305, 5526, 5314,  5345, 5339, 5308, 5142,
                              5352, 5514, 5625, 5347, 5311, 5374, 5170, 5390]

filename = './befkbhalderstatkode.csv'

def csv_to_np_array(file="./befkbhalderstatkode.csv"): 
    stats_np_array = np.genfromtxt(file, delimiter=',', dtype=np.uint, skip_header=1)
    return stats_np_array

def sum_of_people(file=csv_to_np_array()):
    
    english_mask = (np.in1d(file[:,3], english_speaking_countries))
    others_mask = (np.in1d(file[:,3], english_speaking_countries, invert=True))

    english_sum = "Amount of native English speakers:", np.sum(file[english_mask][:,4])
    others_sum = "Amount of people with a native tongue, different to English:", np.sum(file[others_mask][:,4])
    return english_sum, others_sum

def filter_2d_array(file, mask):
    return file[mask]

def get_population_from_x(data, x):
    x_values = np.unique(data[:,x])
                 #For each of the values in the unique array, we'll loop over and see if the x value matches the desired filter 
                 #& then we return the sum for each x value's Y row
    dictValues = {xval:np.sum(data[(data[:,x] == xval)][:,4]) for xval in x_values}
